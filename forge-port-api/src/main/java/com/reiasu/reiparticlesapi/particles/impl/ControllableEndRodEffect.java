...
public class ControllableEndRodEffect implements ControllableParticleEffect {
    private UUID uuid;
    private final boolean faceToPlayer;

    public ControllableEndRodEffect(UUID uuid, boolean faceToPlayer) {
        this.uuid = uuid;
        this.faceToPlayer = faceToPlayer;
    }

    public ControllableEndRodEffect(UUID uuid, boolean faceToPlayer, int ignored, Object ignored2) {
    ...
    @Override
    public ControllableEndRodEffect clone() {
        return new ControllableEndRodEffect(UUID.randomUUID(), faceToPlayer);
    }

    @Override public boolean getFaceToPlayer() { return faceToPlayer; }

    @Override public ParticleType<?> getType() { return ReiModParticles.CONTROLLABLE_END_ROD.get(); }
